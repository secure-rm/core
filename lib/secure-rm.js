...
function secureRmCallback (path, options, callback) {
  if (options.customMethod) {
    rimraf(path, { unlink: options.customMethod }, (err) => callback(err, path))
  } else if (validIDs.includes(options.method)) {
    rimraf(path, { unlink: methods[options.method].method }, (err) => callback(err, path))
  } else {
...
function secureRmPromise (path, options) {
  return new Promise((resolve, reject) => {
    if (options.customMethod) {
      rimraf(path, { unlink: options.customMethod }, (err) => {
        if (err) reject(err)
        else resolve(path)
      })
    } else if (validIDs.includes(options.method)) {
      rimraf(path, { unlink: methods[options.method].method }, (err) => {
        if (err) reject(err)
        else resolve(path)
      })
    } else {
