...
function rimraf (p: string, options, callback) {
  ...
  const afterGlob = (err: NodeJS.ErrnoException | null, results) => {
    if (err)
      return callback(err)
...
function rimraf_ (p: string, options, callback) {
  ...
  fs.lstat(p, (err, stats) => {
    ...
    if (stats && stats.isDirectory())
      return rmdir(p, options, err, callback)
...
function fixWinEPERM(p: string, options, err: NodeJS.ErrnoException | null, callback) {
  fs.chmod(p, 0o666, err2 => {
    if (err2)
      callback(err2.code === 'ENOENT' ? null : err)
    else
      fs.stat(p, (err3, stats) => {
        if (err3)
          callback(err3.code === 'ENOENT' ? null : err)
        else if (stats.isDirectory())
          rmdir(p, options, err, callback)
        else
          options.unlink(p, callback)
      })
  })
}

function fixWinEPERMSync(p: string, options, err: NodeJS.ErrnoException | null) {
  try {
...
function rmdir(p: string, options, originalErr: NodeJS.ErrnoException | null, callback) {
  // try to rmdir first, and only readdir on ENOTEMPTY or EEXIST (SunOS)
  // if we guessed wrong, and it's not a directory, then
  // raise the original error.
  options.rmdir(p, (err) => {
    if (err && (err.code === 'ENOTEMPTY' || err.code === 'EEXIST' || err.code === 'EPERM'))
      rmkids(p, options, callback)
...
function rmkids (p: string, options, callback) {
  fs.readdir(p, (err, files) => {
    if (err)
      return callback(err)
    let n = files.length
    if (n === 0)
      return options.rmdir(p, callback)
    let errState
    files.forEach(file => {
...
function rmkidsSync (p: string, options) {
