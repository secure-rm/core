 ...
 */
export async function init (fileName: string, eventEmitter: events.EventEmitter, alreadyInit?: boolean): Promise<FileData> {
  const fileSize = (await fs.stat(fileName)).size
  const fd = await fs.open(fileName, 'r+')
  if (!alreadyInit) eventEmitter.emit('init', fileName)
  return { fd, fileName, fileSize, eventEmitter }
}
 ...
 * @param fileName The file that will be processed.
 */
export async function mark (fileName: string, eventEmitter: events.EventEmitter) {
 ...
 * @param fileData Generated by function init.
 */
export async function rename ({ fd, fileName, eventEmitter }: FileData) {
  await fs.close(fd)
  ...
  return init(newPath, eventEmitter, true)
}
