...
export async function init (fileName: string): Promise<FileData> {
  emit('info', fileName, 'Initialization')
  const fileSize = (await fs.stat(fileName)).size
  const fd = await fs.open(fileName, 'r+')
  emit('trace', { fd, fileName, fileSize }, 'Initialization')
...
export async function end ({ fd, fileName, fileSize }: FileData) {
  ...
  emit('info', fileName, 'Done')
  emit('trace', { fd, fileName, fileSize }, 'Done')
}
 ...
 * @param fileData Generated by function init.
 */
export async function markFile ({ fileName }: FileData) {
  emit('mark', fileName)
}
...
export async function random ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Writing random data')
...
export async function zeros ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Writing zeros')
...
export async function ones ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Writing ones')
...
export async function byte ({ fd, fileName, fileSize }: FileData, { passes = 1, data }: ByteOptions) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, `Writing 0x${data.toString(16)}`)
...
export async function byteArray ({ fd, fileName, fileSize }: FileData, { passes = 1, data }: ByteArrayOptions) {
  ...
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, `Writing ${data.map(x => '0x' + x.toString(16))}`)
...
export async function forByte ({ fd, fileName, fileSize }: FileData, { initial, condition, increment }: ForByteOptions) {
  for (let i = initial; condition(i); i = increment(i)) {
    emit('debug', fileName, `Writing 0x${i.toString(16)} `)
...
export async function randomByte ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  ...
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Writing random byte')
    await writeExtended(fd, fileSize, 0, async bufferSize => Buffer.alloc(bufferSize, data))
  }
}
...
export async function complementary ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Writing binary complement')
...
export async function rename ({ fd, fileName }: FileData) {
  ...
  emit('debug', fileName, `Renaming to ${newName}`)
...
export async function truncate ({ fd, fileName, fileSize }: FileData, { passes = 1 }: Options = {}) {
  for (let i = 0; i < passes; i++) {
    emit('debug', fileName, 'Truncating')
...
export async function resetTimestamps ({ fd, fileName }: FileData) {
  emit('debug', fileName, 'Resetting file timestamps')
...
export async function changeTimestamps ({ fd, fileName }: FileData, { date1 = new Date(0), date2 = new Date() }: RandomTimestampsOptions = {}) {
  emit('debug', fileName, 'Changing file timestamps')
...
export async function writeExtended (fd: number, size: number, pos: number, getBuffer: (bufferSize: number, pos: number) => Promise<Buffer>): Promise<void> {
  emit('trace', { fd, size, pos }, 'Write data')
