 ...
 */
export async function random ({ fd, fileSize }: FileData, { passes = 1 } = {}) {
  for (let i = 0; i < passes; i++) {
    await writeExtended(fd, fileSize, 0, async bufferSize => crypto.randomBytes(bufferSize))
  }
}
 ...
 * @param passes The number of times the function is executed.
 */
export async function zeros ({ fd, fileSize }: FileData, { passes = 1 } = {}) {
  for (let i = 0; i < passes; i++) {
 ...
 * @param passes The number of times the function is executed.
 */
export async function ones ({ fd, fileSize }: FileData, { passes = 1 } = {}) {
  for (let i = 0; i < passes; i++) {
 ...
 * @param passes The number of times the function is executed.
 */
export async function randomByte ({ fd, fileSize }: FileData, { passes = 1 } = {}) {
 ...
 * @param passes The number of times the function is executed.
 */
export async function complementary ({ fd, fileSize }: FileData, { passes = 1 } = {}) {
  for (let i = 0; i < passes; i++) {
